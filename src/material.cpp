...
Material::Material(std::optional<MaterialConfig> material_config): Asset(__FUNCTION__), material_config_(material_config) {}
...
void Material::Use() {
    ...
    if (Exists(MaterialUniform::SpecularIntensity))
        glUniform1f(GetUniform(MaterialUniform::SpecularIntensity), material_config_->specular_intensity);
    if (Exists(MaterialUniform::Shininess))
        glUniform1f(GetUniform(MaterialUniform::Shininess), material_config_->shininess);
}
