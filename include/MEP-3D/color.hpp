...
class Color {
public:
	constexpr Color(Uint8 r, Uint8 g, Uint8 b, Uint8 a = 255): r_(r), g_(g), b_(b), a_(a) {}

	constexpr Uint8 R() const {
		return r_;
	}
	constexpr Uint8 G() const {
		return g_;
	}
	constexpr Uint8 B() const {
		return b_;
	}
	constexpr Uint8 A() const {
		return a_;
	}
	...
	constexpr float Af() const {
		return static_cast<float>(a_) / 255.0f;
	}
private:
	Uint8 r_;
	Uint8 g_;
